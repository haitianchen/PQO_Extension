...
#define WRITE_LOCATION_FIELD_SR(fldname) \
    (node->fldname < -999 ? appendStringInfo(str, " :" CppAsString(fldname) " %d", node->fldname) : appendStringInfo(str, " :" CppAsString(fldname) " %d", -1))

/* Write a Node field */
#define WRITE_NODE_FIELD(fldname)                                \
    (appendStringInfoString(str, " :" CppAsString(fldname) " "), \
     outNode_sr(str, node->fldname))
...
_outPlannedStmt(StringInfo str, const PlannedStmt *node)
{
    ...
    WRITE_LOCATION_FIELD_SR(stmt_location);
    WRITE_LOCATION_FIELD_SR(stmt_len);
}
...
_outRangeVar(StringInfo str, const RangeVar *node)
{
    ...
    WRITE_LOCATION_FIELD_SR(location);
}
 ...
 */
void outNode_sr(StringInfo str, const void *obj)
{
    if (obj == NULL)
        appendStringInfoString(str, "<>");
 ...
 */
char *
nodeToString_sr(const void *obj)
{
    StringInfoData str;
